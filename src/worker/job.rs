...
Report when the job is complete or if you encounter issues you cannot resolve."#,
    ...
    async fn execute_plan(
        ...
        // BEGIN @FORK linoasclaw: Hook: TransformResponse — allow hooks to intercept worker responses
        let response = {
...
impl<'a> LoopDelegate for JobDelegate<'a> {
    ...
    async fn handle_text_response(
        ...
        let text = {
            ...
            let event = HookEvent::ResponseTransform {
                user_id,
                thread_id: self.worker.job_id.to_string(),
                response: text.to_string(),
            };
            match self.worker.deps.hooks.run(&event).await {
                Ok(HookOutcome::Continue { modified: Some(r) }) => {
                    text_owned = r;
                    text_owned.as_str()
                }
                _ => text,
